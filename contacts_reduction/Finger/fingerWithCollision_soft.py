...
def createScene(rootNode):
    ...
    rootNode.addObject('LocalMinDistance', name="Proximity", alarmDistance="8.0", contactDistance="0.5", angleCone="0.5")
    ...
    cube.addObject('MeshOBJLoader', name="loader", filename="mesh/smCube27.obj",scale="3.0", translation=[-60,40,7.5])
    ...
    cube.addObject('PositionConstraint', indices=list(range(0, 27)),
                   valueType="displacement", value=0, useDirections=[1, 0, 0])
    
    finger = solverNode.addChild('finger')
    finger.addObject('MeshVTKLoader', name="loader", filename=path+'fingerFine.vtk')
    ...
    finger.addObject('TetrahedronFEMForceField', poissonRatio="0.4", youngModulus="70")
    ...
    finger.addObject('BoxROI', name='Softskin', box=[5, 0, 0, -105, 10, 15], drawBoxes=True)
    ...
    modelSubTopo.addObject('TriangleFEMForceField', template='Vec3d', name='Append_subTopoFEM', method='large', poissonRatio='0.49',  youngModulus='700')

    modelCollis = finger.addChild('modelCollis')
    modelCollis.addObject('MeshSTLLoader', name='loader', filename=path+ r'/fingerFine.stl')
    ...
    modelVisu.addObject('MeshSTLLoader', name='loader', filename=path + r'/finger.stl')
